...
// Demo data utilities for Phase 2

import { CoacheeCanonicalProfile } from "@/lib/types/canonical-json"

// Sample profile for demo purposes
export const DEMO_PROFILE: CoacheeCanonicalProfile = {
  "participant": {
...
export async function setupDemoData() {
  try {
    ...
    const response = await fetch("/api/coachee-profiles", {
      method: "POST",
    ...
    if (response.ok) {
      console.log("Demo profile loaded successfully")
    } else {
      console.error("Failed to load demo profile")
    }
  } catch (error) {
